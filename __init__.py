...
class JCH_TensorInspector:
	...
	def get_shape_interpretation(self, shape):
		"""全量常数推导：锁定核心带宽识别"""
		s = list(shape)
		standard_h = {
		...
		if len(s) >= 2:
			width = s[-1]
			if width == 12288: return f"🧬 融合 QKV 空间 (3.0x) | 源自 4096 核心 (Qwen3)"
			for h, name in standard_h.items():
				if width == h * 3: return f"🧬 融合 QKV 投影 (3.0x) | 源自 {name}"
				if width == h * 4: return f"🚀 MLP 膨胀空间 (4.0x) | 源自 {name}"
				if width == h * 3.5: return f"🚀 MLP 膨胀空间 (3.5x) | 源自 {name}"
			if width in standard_h: return f"标准语义特征层 ({standard_h[width]})"
		return f"💡 结构: {self.format_shape(s)}"
	...
	def inspect_tensor_stats(self, tensor, log, prefix="  "):
		...
		if tensor.is_floating_point():
			try:
				v_min, v_max = torch.min(tensor).item(), torch.max(tensor).item()
				v_mean = torch.mean(tensor.float()).item()
				v_nan = torch.isnan(tensor).any().item()
	...
	def inspect_data(self, anything, label):
		...
		if hasattr(anything, 'first_stage_model') or 'VAE' in str(type(anything)):
			...
			if hasattr(diff, "input_blocks"):
				...
				try:
					...
					if hasattr(first_conv, "weight"):
						temp_log.append(f"  ├─ 卷积入口门(conv_in): {self.format_shape(first_conv.weight.shape, is_weight=True)}")
				except: pass
				...
				try:
					...
					if hasattr(b0, "x_block") and hasattr(b0.x_block, "attn"):
						...
						temp_log.append(f"  ├─ 🔬 [核心车间 0号抽检]: 融合 QKV 墙: {self.format_shape(qkv.weight.shape, is_weight=True)}")
						# 新增：探测 38 个专家属性
				...
				try:
					...
					if hasattr(b0, "img_attn"):
						qkv = b0.img_attn.qkv
						temp_log.append(f"  ├─ 🔬 [核心车间 0号抽检]: 图像 QKV 墙: {self.format_shape(qkv.weight.shape, is_weight=True)}")
			...
			for attr in ["img_in", "x_embedder"]:
				if hasattr(diff, attr):
					...
					if hasattr(target, 'weight'):
						temp_log.append(f"  ├─ 图像入口门({attr}): {self.format_shape(target.weight.shape, is_weight=True)}")
					else:
			...
			for attr in ["txt_in", "context_embedder"]:
				if hasattr(diff, attr):
					...
					if hasattr(target, 'weight'):
						temp_log.append(f"  ├─ 文本入口门({attr}): {self.format_shape(target.weight.shape, is_weight=True)}")
					break
			...
			for attr in ["final_layer", "output_layer"]:
				if hasattr(diff, attr):
					...
					target = gate.linear if hasattr(gate, 'linear') else gate
					if hasattr(target, 'weight'):
						temp_log.append(f"  ├─ 图像出口门({attr}): {self.format_shape(target.weight.shape, is_weight=True)}")
					break
		...
		# --- B: 潜空间字典探测 (保留 V21 所有细节) ---
			...
			if hasattr(v_samples, 'shape'):
				temp_log.append(f"  ├─ 核心 Latent: {self.format_shape(v_samples.shape, is_weight=False)}")
			...
			for k, v in anything.items():
				if k != 'samples' and hasattr(v, 'shape'):
					...
					temp_log.append(f"  ├─ {icon} '{k}': {self.format_shape(v.shape, is_weight=False)}")
			...
			try:
				if len(anything) > 0 and isinstance(anything[0], (list, tuple)):
					...
					if hasattr(main_t, 'shape'):
						temp_log.append(f"  ├─ 核心语义矩阵: {self.format_shape(main_t.shape, is_weight=False)}")
						...
						for k,v in meta.items():
							...
							if hasattr(v, 'shape'):
								...
								for idx, item in enumerate(v):
									if hasattr(item, 'shape'): temp_log.append(f"    │  └─ 样板[{idx}]: {self.format_shape(item.shape, is_weight=False)}")
			except: pass
		...
		# --- D: 通用张量审计 ---
			...
			temp_log.append(f"📐 尺寸结构: {self.format_shape(anything.shape, is_weight=False)}")
